...
def process_amass(smooth_n=4):
    ...
    b = 0
    out_pose, out_pose_flat, out_shape, out_tran, out_joint, out_vrot, out_vacc = [], [], [], [], [], [], []
    for i, l in tqdm(list(enumerate(length))):
        ...
        out_pose_flat.append(pose[b:b + l].clone().reshape(-1, 72))  # N, 72
    ...
    torch.save({'acc': out_vacc, 'ori': out_vrot, 'pose': out_pose_flat, 'tran': out_tran}, os.path.join(paths.amass_dir, 'train.pt'))
...
def process_dipimu():
    ...
    train_split = ['s_0{}'.format(i) for i in range(1, 9)]
    os.makedirs(paths.dipimu_dir, exist_ok=True)

    def create_split_set(split, name):
        ...
        torch.save({'acc': accs, 'ori': oris, 'pose': poses, 'tran': trans},
                   os.path.join(paths.dipimu_dir, '{}.pt'.format(name)))

    create_split_set(test_split, 'test')
    create_split_set(train_split, 'train')
...
if __name__ == '__main__':
    ...
    # data = torch.load(os.path.join(paths.amass_dir, 'test.pt'))
    # out_vacc = data['acc']
    # out_vrot = data['ori']
    # out_pose = data['pose']
    # out_tran = data['tran']
    # print('Synthetic AMASS dataset is saved at', paths.amass_dir)
    # print(out_vacc[0].shape, out_vrot[0].shape, out_pose[0].shape, out_tran[0].shape)
    #
    # dipimu = torch.load(os.path.join(paths.dipimu_dir, 'test.pt'))
    # accs = dipimu['acc']
    # oris = dipimu['ori']
    # poses = dipimu['pose']
    # trans = dipimu['tran']
    # print(accs[0].shape, oris[0].shape, poses[0].shape, trans[0].shape)
